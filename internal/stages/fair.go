...
// smoothPolylineScaled runs the smoothing kit with its Chaikin
// corner-cut cap scaled by s — street-running modes pass s < 1 so real
// street corners survive.
func smoothPolylineScaled(l *geo.Line, s float64) *geo.Line {
	return smoothPolylineOnce(l, s)
}

func smoothPolylineOnce(l *geo.Line, s float64) *geo.Line {
	pts := l.Pts
	if len(pts) < 3 {
		return l
	}
	...
	// corner fillets are GONE: replacing a turn cluster with a tangent
	// circular arc tightened long gentle curves into compass corners —
	// the 9°-per-joint trigger meant to catch chord-straightened street
	// corners also caught every sweeping subway curve (Beekman St on the
	// A/C, Chrystie St on the B/D) and redrew it as straight–arc–straight
	// tens of metres off the steel. The capped Chaikin below is the whole
	// smoothing kit now; faceted 90° street corners are the cheaper flaw.
